...
class Html2Text {
    ...
    public static function convert($html, $ignore_error = false) {
        ...
        $output = static::replaceNtoBR($output);
        return $output;
    }

    static function replaceNtoBR($text) {
        // replace \r\n to \n
        $text = str_replace("\n", "<br>", $text);
        //$text = preg_replace('/(<br>)\1+/', '<br>', $text);
        //delete <br> at the begining of text
    ...
    static function iterateOverNode($node, $prevName = null, $in_pre = false, $is_office_document = false) {
        ...
        switch ($name) {
            ...
            case "pre":
                ...
                break;
            case "ol":
                $output = "<ol start='{$node->getAttribute("start")}'>";
                break;
            case "ul":
                $output = "<ul>";
                break;
            ...
            case "p":
                ...
                if(isset($node->previousSibling) && substr($node->previousSibling->nodeName,0,1)==='h')
                {
                    $output = "\n";
                    break;
                }
                ...
                if ($href == null) {
                    ...
                    if ($href == $output || $href == "mailto:$output" || $href == "http://$output" || $href == "https://$output") {
                        ...
                        if ($output) {
                            ...
                            $output = "<a href='$href' target='_blank'>$output</a>";
                        } else {
                            // empty string
                            $output = "";//$href;
                        }
