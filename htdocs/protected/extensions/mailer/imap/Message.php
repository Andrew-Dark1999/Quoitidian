...
class Message
{
     ...
     */
    public function __construct($imapStream, $uid, $attachment_file_path)
    {
        $this->_attachment_file_path = $attachment_file_path;
        ...
        if($this->loadMessage() !== true)
            return false;
//            throw new \RuntimeException('Message with ID ' . $uid . ' not found.');
    }
    ...
    protected function processStructure($structure, $partIdentifier = null)
    {
        ...
        if ((isset($parameters['name']) || isset($parameters['filename']))
            ...
            $attachment = new Attachment($this->imapStream, $this, $structure, $partIdentifier);
            $attachment->saveToDirectory($this->_attachment_file_path);
                ...
                if (function_exists('mb_convert_encoding')) {
                    if (!in_array($parameters['charset'], mb_list_encodings())) {
                        if ($structure->encoding === 0) {
                            $parameters['charset'] = mb_detect_encoding($messageBody);
