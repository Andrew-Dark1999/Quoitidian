...
class Attachment
{
     ...
     */
    public function __construct($imapStream, Message $message, $structure, $partIdentifier = null)
    {
    ...
    public function getAttachment(){
        return array(
            'origin_file_name' => $this->filename,
    ...
    public function getUploadFileName(){
        return $this->messageId . '_' . $this->filename;
    }
    ...
    {
        $path = rtrim($path, '/') . '/';
        $this->upload_file_name = $this->messageId . '_' . $this->getFileName();
        if (is_dir($path)){
            try{
                return $this->saveAs($path . $this->upload_file_name);
            } catch(Exception $e){
                return false;
            }
        }
