...
class ThumbNail{
	...
	private function thumbmaker() {
		...
		if($this->loadimage()) {
			...
			if ($this->SaveProportional){ $this->proportional_coords(); }
            ...
            imagecopyresampled($this->thumb,
                               $this->im,
                               $this->Framewidth*($this->frame_offset-1),
                               $this->Framewidth,
                               0,
                               0,
                               $this->thumbx-($this->frame_offset*$this->Framewidth)-$this->shadow_offset,
                               $this->thumby-2*$this->Framewidth-$this->shadow_offset,
                               imagesx($this->im),
                               imagesy($this->im)
            );
    ...
    /**
     * Set proporcional thumb height
     */
    private function proportional_coords() {
        $const = 1;
        if($this->size[0] != $this->size[1]) $const = $this->size[1] / $this->size[0];
        
        $this->Thumbheight = round($this->Thumbwidth * $const);
    }
	...
	private function square() {
		...
		$squaresize = min($this->thumbx,$this->thumby);
