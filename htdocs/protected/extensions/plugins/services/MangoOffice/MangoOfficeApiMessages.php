...
/**
 * Class MangoOfficeApiMessages - обработка уведомлений сервера
 */



class MangoOfficeApiMessages{

    private $_api_result;

    protected $_message_code_success = 1000;    // int|array. Код(ы) положительных сообщения для $this->isSuccess() == true
    ...
    public function setApiResult($api_result){
        if($api_result && is_string($api_result)){
    ...
    private function getCodeMessage(){
        if($this->_api_result && array_key_exists('result', $this->_api_result)){
            $code = (int)$this->_api_result['result'];
        }

        if($this->isCodeMessage($code) == false){
            $code;
        }
    ...
    private function isCodeMessage($mesage_code){
        return array_key_exists($mesage_code, $this->_messages);
    }
    ...
    public function isSuccess(){
        $message_code_success = (array)$this->_message_code_success;
        $code_message = $this->getCodeMessage();

        if($code_message && $message_code_success && in_array($code_message, $message_code_success)){
            return true;
        }
