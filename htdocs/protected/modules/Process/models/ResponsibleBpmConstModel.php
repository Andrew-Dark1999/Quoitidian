<?php
/**
 * ResponsibleBpmConstModel - контроль сонстант, что выбраны в качестве ответсвенного за процесс
 *
 * @autor Alex R.
 */
...
class ResponsibleBpmConstModel extends ResponsibleBpmFactoryModel{
    ...
    protected function actionCheck(){
        if($this->_vars['ug_type'] == \ParticipantModel::PARTICIPANT_UG_TYPE_CONST){
    ...
    public function getLableTitle(){
        ...
        $label = (new \ParticipantConstModel())->getTypeConstTitle($this->_vars['base_ug_id']);

        return \Yii::t('ProcessModule.base', $label);
    }
